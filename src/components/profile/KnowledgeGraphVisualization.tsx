    ...
    return (
      <div className="flex h-full min-h-[400px] w-full items-center justify-center rounded-xl border border-white/10 bg-black/40 backdrop-blur-md">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-gray-400">Loading neural network...</p>
        </div>
      </div>
    );
  }
        ...
        <ForceGraph2D
          ...
          nodeCanvasObject={(node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
            ...
            if (highlighted) {
              ...
              const gradient = ctx.createRadialGradient(node.x, node.y, r * 0.8, node.x, node.y, glowRadius);
              gradient.addColorStop(0, hexToRgba(node.color, isDark ? 0.4 : 0.25));
              gradient.addColorStop(0.2, hexToRgba(node.color, isDark ? 0.4 : 0.25));
            ...
            if (globalScale > 1.2 && highlighted) {
              ...
              ctx.fillStyle = isDark ? 'rgba(15, 17, 23, 0.85)' : 'rgba(255, 255, 255, 0.92)';
